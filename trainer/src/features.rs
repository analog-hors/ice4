    ...
    pub fn extract(&mut self, board: &Board) {
        ...
        for &color in &Color::ALL {
            for square in board.pieces(Piece::Pawn) & board.colors(color) {
                ...
                self.passer_own_king_dist += own_king_dist as f32 * inc;
                self.passer_own_king_dist_squared += (own_king_dist * own_king_dist / 3) as f32 * inc;

                let enemy_king_dist = king_dist(!color);
                self.passer_enemy_king_dist += enemy_king_dist as f32 * inc;
                self.passer_enemy_king_dist_squared += (enemy_king_dist * enemy_king_dist / 3) as f32 * inc;
            }
        }
