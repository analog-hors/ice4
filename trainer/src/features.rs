...
pub fn extract_features(board: &Board, linear: &mut LinearFeatures, king_safety: &mut KingSafetyFeatures, phase: &mut f32) {
    for &piece in &Piece::ALL {
        for unflipped_square in board.pieces(piece) {
            ...
            linear.mobility[piece as usize] += inc * mob.len() as f32;

            let king_ring_attacks = (get_king_moves(board.king(!color)) & mob).len();
            if piece != Piece::King && king_ring_attacks > 0 {
                king_safety.king_attack_weight[color as usize][piece as usize] += king_ring_attacks as f32;
            }
        }
    }
    ...
    for &color in &Color::ALL {
        for square in board.pieces(Piece::Pawn) & board.colors(color) {
            ...
            linear.passer_own_king_dist[king_dist(color)] += inc;
            linear.passer_enemy_king_dist[king_dist(!color)] += inc;
        }
    }
    ...
    for color in Color::ALL {
        ...
        for phalanx in phalanx_pawns {
            linear.phalanx_pawn_rank[phalanx.rank().relative_to(color) as usize - 1] += inc;
        }
