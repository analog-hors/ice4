...
struct Board {
    ...
    void movegen(Move list[], int& count, int quiets, int& mobility) {
        ...
        for (int sq = A1; sq <= H8; sq++) {
            ...
            for (int i = 0; i < 8; i++) {
                king_ring[king_sq[stm == WHITE] + RAYS[i]] = KING_ATTACK_WEIGHT[piece];
            }
            ...
            if (piece == PAWN) {
                ...
                if (!board[sq + dir]) {
                    ...
                    attack_weight += king_ring[sq + dir];
                    ...
                    if (board[sq - dir - dir] == INVALID && !board[sq + dir + dir]) {
                        mobility += MOBILITY[piece];
                        attack_weight += king_ring[sq + dir+dir];
                        if (quiets) {
                ...
                if (ep_square == sq + dir-1 || board[sq + dir-1] & OTHER && ~board[sq + dir-1] & stm) {
                    mobility += MOBILITY[piece];
                    attack_weight += king_ring[sq + dir-1];
        ...
        mobility += attack_weight * attack_weight / 160;
