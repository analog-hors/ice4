    ...
    void movegen(Move list[], int& count, int quiets, int& mobility) {
        ...
        for (int sq = A1; sq <= H8; sq++) {
            ...
            if (piece == PAWN) {
                ...
                if (!board[sq + dir]) {
                    mobility += MOBILITY[piece] + in_enemy_king_ring(sq + dir) * KING_RING_ATTACKS;
                    ...
                    if (board[sq - dir - dir] == INVALID && !board[sq + dir + dir]) {
                        mobility += MOBILITY[piece] + in_enemy_king_ring(sq + dir+dir) * KING_RING_ATTACKS;
                        if (quiets) {
                ...
                if (ep_square == sq + dir-1 || board[sq + dir-1] & OTHER && ~board[sq + dir-1] & stm) {
                    mobility += MOBILITY[piece] + in_enemy_king_ring(sq + dir-1) * KING_RING_ATTACKS;
                ...
                for (int i = STARTS[piece]; i < ENDS[piece]; i++) {
                    ...
                    for (int j = 0; j < LIMITS[piece]; j++) {
                        ...
                        mobility += MOBILITY[piece] + in_enemy_king_ring(raysq) * KING_RING_ATTACKS;
