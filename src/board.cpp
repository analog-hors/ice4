...
struct Board {
    ...
    void movegen(Move list[], int& count, int quiets, int& mobility) {
        ...
        for (int sq = A1; sq <= H8; sq++) {
            ...
            int init_king_attack_power = king_attack_power;
            for (int i = 0; i < 8; i++) {
                if (attacked[king_sq[stm == WHITE] + RAYS[i]]) {
                    attacked[king_sq[stm == WHITE] + RAYS[i]] = 0;
                    king_attack_power += KING_ATTACK_POWER[piece];
                }
            }
            king_attack_count += init_king_attack_power < king_attack_power;
        }

        mobility += king_attack_power * KING_ATTACK_SCALE[king_attack_count > 7 ? 7 : king_attack_count];
