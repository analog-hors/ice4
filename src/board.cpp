...
struct Board {
    ...
    void movegen(Move list[], int& count, int quiets, int& mobility) {
        ...
        for (int sq = A1; sq <= H8; sq++) {
            ...
            int init_attacked_squares = attacked_squares;
            ...
            if (piece == PAWN) {
                ...
                if (!board[sq + dir]) {
                    ...
                    attacked_squares += king_ring[sq + dir] * KING_ATTACKED_SQUARES[piece];
                    ...
                    if (board[sq - dir - dir] == INVALID && !board[sq + dir + dir]) {
                        mobility += MOBILITY[piece];
                        attacked_squares += king_ring[sq + dir+dir] * KING_ATTACKED_SQUARES[piece];
                        if (quiets) {
                ...
                if (ep_square == sq + dir-1 || board[sq + dir-1] & OTHER && ~board[sq + dir-1] & stm) {
                    mobility += MOBILITY[piece];
                    attacked_squares += king_ring[sq + dir-1] * KING_ATTACKED_SQUARES[piece];
                ...
                for (int i = STARTS[piece]; i < ENDS[piece]; i++) {
                    ...
                    for (;;) {
                        ...
                        mobility += MOBILITY[piece];
                        attacked_squares += king_ring[raysq] * KING_ATTACKED_SQUARES[piece];
            ...
            if (attacked_squares > init_attacked_squares) {
                attackers += KING_ATTACKERS[piece];
            }
        }
        mobility += (attackers * attackers + attacked_squares * attacked_squares) / 160;
        #undef OTHER
    }
