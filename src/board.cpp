        ...
        for (int sq = A1; sq <= H8; sq++) {
            ...
            if (piece == PAWN) {
                ...
                if (!board[sq + dir]) {
                    ...
                    piece_attacks += king_ring[sq + dir];
                    ...
                    if (board[sq - dir - dir] == INVALID && !board[sq + dir + dir]) {
                        mobility += MOBILITY[piece];
                        piece_attacks += king_ring[sq + dir+dir];
                        if (quiets) {
                ...
                if (ep_square == sq + dir-1 || board[sq + dir-1] & OTHER && ~board[sq + dir-1] & stm) {
                    mobility += MOBILITY[piece];
                    piece_attacks += king_ring[sq + dir-1];
            ...
            mobility += KING_RING_ATTACKS[piece_attacks];
        }
        #undef OTHER
    }
