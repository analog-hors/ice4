...
struct Searcher {
    ...
    int negamax(Board &board, Move &bestmv, int alpha, int beta, int signed_depth, int ply) {
        ...
        if (!pv && !board.check && depth && depth < 7 && eval >= beta + 44 * depth) {
            return eval;
        }
        ...
        if (!pv && !board.check && depth && depth < 4 && eval <= alpha - 50 * depth - 250) {
        ...
        for (int i = 0; i < mvcount; i++) {
            ...
            if (v >= beta) {
                if (!victim) {
                    int bonus = signed_depth * signed_depth << ((eval <= alpha) + (eval <= alpha - 30));
                    int16_t *hist;
                    for (int j = 0; j < i; j++) {
        ...
        if ((depth || best != eval) && best > LOST + ply) {
            ...
            if (!board.board[bestmv.to] && (
                ...
                double weight = min(signed_depth * signed_depth + 3, 95) / 689.0;
