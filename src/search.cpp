...
int IIR_MIN_DEPTH = 3;
int IIR_REDUCTION = 1;
int CORRHIST_PAWN_DIV = 228;
int CORRHIST_MATERIAL_DIV = 228;
int CORRHIST_COUNTERMOVE_DIV = 228;
int RFP_MARGIN = 44;
int RFP_MAX_DEPTH = 7;
int NMP_MIN_DEPTH = 1;
int NMP_FACTOR = 29;
int NMP_CONSTANT = 242;
int NMP_DIVIDOR = 88;
int RAZOR_MAX_DEPTH = 4;
int RAZOR_MARGIN = 50;
int RAZOR_CONSTANT = 250;
int IID_MIN_DEPTH = 2;
int IID_REDUCTION = 6;
...
int HIST_EVAL_MARGIN = 30;
int CORRHIST_UPDATE_CONSTANT = 3;
int CORRHIST_UPDATE_MAX = 95;
double CORRHIST_UPDATE_DIVISOR = 689;
double TM_HARD = 0.0004;
double TM_SOFT = 0.000043;
int ASP_INITIAL = 7;
double ASP_WIDEN = 1.8;
...
struct Searcher {
    ...
    int negamax(Board &board, Move &bestmv, int alpha, int beta, int depth, int ply) {
        ...
        evals[ply] = board.eval(mobilities[ply+1] - mobilities[ply] + TEMPO)
            + corr_hist[board.stm != WHITE][board.pawn_hash % CORR_HIST_SIZE] / CORRHIST_PAWN_DIV
            + corr_hist[board.stm != WHITE][board.material_hash % CORR_HIST_SIZE] / CORRHIST_MATERIAL_DIV
            + (*conthist_stack[ply+1])[0][0] / CORRHIST_COUNTERMOVE_DIV;
        ...
        if (!pv && !board.check && depth > 0 && depth < RFP_MAX_DEPTH && eval >= beta + RFP_MARGIN * depth) {
            return eval;
        }
        ...
        if (!pv && !board.check && eval >= beta && beta > -20000 && depth > NMP_MIN_DEPTH) {
            Board mkmove = board;
            ...
            int reduction = (eval - beta + depth * NMP_FACTOR + NMP_CONSTANT) / NMP_DIVIDOR;
        ...
        if (!pv && !board.check && depth > 0 && depth < RAZOR_MAX_DEPTH && eval <= alpha - RAZOR_MARGIN * depth - RAZOR_CONSTANT) {
        ...
        for (int j = 0; j < mvcount; j++) {
            if (hashmv.from == moves[j].from && hashmv.to == moves[j].to) {
                ...
                score[j] = history[board.board[moves[j].from]][moves[j].to]
                    ...
                    + COUNTER_HIST_FACTOR * (*conthist_stack[ply + 1])[board.board[moves[j].from]][moves[j].to]
                    ...
                    + FOLLOWUP_HIST_FACTOR * (*conthist_stack[ply])[board.board[moves[j].from]][moves[j].to];
            }
        }
        ...
        int quiets_to_check = pv ? -1 : (depth*depth + LMP_CONSTANT) >> (!improving + 1);
        ...
        for (int i = 0; i < mvcount; i++) {
            ...
            if (v >= beta) {
                if (!victim) {
                    int bonus = int(HIST_QUAD_UPDATE * depth * depth + HIST_LINEAR_UPDATE * depth) << ((eval <= alpha) + (eval <= alpha - HIST_EVAL_MARGIN));
                    int16_t *hist;
                    for (int j = 0; j < i; j++) {
        ...
        if ((depth > 0 || best != eval) && best > LOST + ply) {
            ...
            if (!board.board[bestmv.to] && (
                ...
                double weight = min(depth * depth + CORRHIST_UPDATE_CONSTANT, CORRHIST_UPDATE_MAX) / CORRHIST_UPDATE_DIVISOR;
    ...
    #define MAX_DEPTH 200
        ...
        hard_limit = now() + time_alotment * TM_HARD;
        soft_limit = now() + time_alotment * TM_SOFT;
        Move mv;
        int v = 0;
        try {
